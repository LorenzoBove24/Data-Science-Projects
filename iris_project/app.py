...
# --- 1. SETUP INIZIALE ---
# Carichiamo i nomi delle classi (setosa, versicolor, virginica)
iris_names = load_iris().target_names 
# Carica lo scaler
try:
    scaler = joblib.load('scaler.pkl')
except FileNotFoundError:
    st.error("Manca il file 'scaler.pkl'. Esegui di nuovo il training salvando anche lo scaler.")
    st.stop()
...
# Usiamo @st.cache_resource per evitare di ricaricare il file a ogni click
@st.cache_resource
...
if model_choice == "Random Forest":
    file_to_load = 'modello_rf.pkl'
else:
    file_to_load = 'modello_knn.pkl'

clf = load_model(file_to_load)

if clf is None:
    st.error(f"Errore: File '{file_to_load}' non trovato. Controlla la cartella.")
    st.stop()
else:
