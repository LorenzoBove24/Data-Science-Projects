...
class LLMBackend:
    ...
    Unified interface for LLM providers:
      ...
      - huggingface  → HuggingFaceEndpoint (HUGGINGFACEHUB_API_TOKEN)
      - groq         → ChatOpenAI con base_url Groq (GROQ_API_KEY)
    """
    ...
    # GROQ
    # ------------------------------------------------------------------
    def _build_groq_chat(self) -> Optional[BaseChatModel]:
        # Groq uses the same client as OpenAI — only change base_url and API key.
        api_key = os.getenv("GROQ_API_KEY")
        if not api_key:
            print("[LLMBackend] GROQ_API_KEY non trovata nel .env")
            return None
