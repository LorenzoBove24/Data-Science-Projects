...
# ---------------------------------------------------------------------
# Radio button per selezionare la modalità agente
        ...
        if show_sources and docs:
            with st.expander("📎 Sources used"):
                for i, d in enumerate(docs):
                    ...
                    sim_score = d.metadata.get("similarity_score", None)
                    prefix = f"[DB: {db_name}] " if db_name else ""
                    sim_str = f" | Similarity: {sim_score:.3f}" if sim_score is not None else ""
                    st.markdown(f"**Source {i+1}:** {prefix}`{src}`{sim_str}")
