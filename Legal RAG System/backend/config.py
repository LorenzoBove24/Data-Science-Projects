...
def _find_all_vector_stores(base_dir: str) -> List[str]:
    """Scan base_dir and returns the paths of all the subfolders that were found."""
    base_path = Path(base_dir)
    if not base_path.exists():
        return []
    return [str(p) for p in sorted(base_path.iterdir()) if p.is_dir()]
...
class RAGConfig:
    ...
    # ---------------- LLM ----------------
    # "groq"       -> ChatOpenAI (needs GROQ_API_KEY)
    ...
    # Automatically fills vector_store_dirs with all the stores found on the disk
      if not self.vector_store_dirs:
